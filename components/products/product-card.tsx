...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <Card className="group h-full gap-0 overflow-hidden rounded-2xl bg-white p-0">
      ...
      <div
        className={`relative m-2 h-56 overflow-hidden rounded-xl bg-gradient-to-b ${accent.tint}`}
      >
      ...
      <CardContent className="flex h-full flex-1 flex-col p-5 sm:p-6">
        <div className="flex items-center gap-1.5">
          <span
            className={`h-1.5 w-1.5 rounded-full ${categoryDot[product.category] ?? "bg-neutral-400"}`}
          />
          <span
            className={`text-[11px] font-semibold tracking-[0.1em] ${categoryText[product.category] ?? "text-neutral-400"} uppercase`}
          >
            {product.category}
          </span>
        </div>
